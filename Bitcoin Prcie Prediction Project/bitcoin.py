...
def computeDelta(wt, X, Xi):
    def similarity(x,y):
        ...
        for i in range(0,len(x)):
            temp = temp + ((x.iloc[i] - np.mean(x)) * (y.iloc[i] - np.mean(y)))
        return temp/(len(x)*np.std(x)*np.std(y))
    """
    ...
    for i in range(0, len(Xi)):
        y_i = Xi.iloc[i]
#         print(y_i)
        prob_y_1 += y_i[-1] * np.exp(wt * similarity(x_i, y_i[0:(len(y_i)-1)]))
        prob_y_0 +=  np.exp(wt * similarity(x_i, y_i[0:(len(y_i)-1)]))
    return prob_y_1 / prob_y_0
...
# This constant was not specified in the paper, but we will use 2.
# YOUR CODE HERE
weight = 2  
