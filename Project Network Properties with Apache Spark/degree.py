...
def simple(g):
	...
	# YOUR CODE HERE
	edges = g.edges.select("dst","src").unionAll(g.edges)
	...
	filtered_edges = g.edges.filter("src!=dst")
	return GraphFrame(g.vertices,filtered_edges)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
	count_degree = g.inDegrees.selectExpr('id as id', 'inDegree as degree').groupBy('degree').count()
...
def power_law(distribution):
	counts = [int(row['count']) for row in distribution.collect()]
	#distribution.map(lambda x: x[1]).collect()    
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	pairs = lines.map(lambda x: x.split(delim)).map(lambda x: (int(x[0]), int(x[1])))
	# print(pairs)
	#pairs = lines.map(lambda x: x.strip().split(delim)).map(lambda x: (int(x[0]), int(x[1])))
	# print(pairs)
