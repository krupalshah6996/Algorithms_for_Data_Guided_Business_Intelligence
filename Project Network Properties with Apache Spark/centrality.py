...
def closeness(g):
	...
	group_ID = paths.select(functions.explode('distances')).groupBy('key')
	# Sum by ID
	sum_ID = group_ID.agg(functions.sum('value').alias('sum'))

	# Get the inverses and generate desired dataframe.
	return sum_ID.selectExpr('key as id','1/sum as closeness')
