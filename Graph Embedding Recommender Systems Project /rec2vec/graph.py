...
def records_to_graph():
    ...
    nodedict = {}
    id = 0
    ratings_range = [str(i+1) for i in range(5)]

    # YOUR CODE HERE
    for i, director in enumerate(directors, start = id):
      node = Node(id = i, name = director, type = 'director')
      nodelist.append(node)
      nodedict[director] = node
    id = i

    for i, actor in enumerate(actors, start = id):
      node = Node(id = i, name = actor, type = 'actor')
      nodelist.append(node)
      nodedict[actor] = node
    id = i
    
    for i, genre in enumerate(genres, start = id):
      node = Node(id = i, name = genre, type = 'genre')
      nodelist.append(node)
      nodedict[genre] = node
    id = i

    for i, user in enumerate(ratings.keys(), start = id):
      # print(user)
      node = Node(id = i, name = user, type = 'user')
      nodelist.append(node)
      nodedict[user] = node
    id = i

    for i, key in enumerate(movies.keys(), start = id):
      node = Node(id = i, name = key, type = 'movie')
      nodelist.append(node)
      nodedict[key] = node
    id = i

    for key in movies.keys():
      for i, rating in enumerate(ratings_range, start = id):
        temp_rating = key + "_" + rating
        # print(temp_rating)
        node = Node(id = i, name = temp_rating, type = 'rating')
        nodelist.append(node)
        nodedict[temp_rating] = node
      id = i
    # print(nodedict)
    ...
    for key in ratings.keys():
      # print(key)
      # print(ratings[key])
      for rating_key in ratings[key].keys():
        temp = ratings[key][rating_key]
        # print(temp)
        temp_rating = rating_key + "_" + str(temp)
        nodedict[temp_rating].neighbors.append(nodedict[key])
        nodedict[key].neighbors.append(nodedict[temp_rating])

    for key in movies.keys():
      if movies[key].director:
        nodedict[movies[key].director].neighbors.append(nodedict[key])
        nodedict[key].neighbors.append(nodedict[movies[key].director])
      for actor in movies[key].actors:
        nodedict[actor].neighbors.append(nodedict[key])
        nodedict[key].neighbors.append(nodedict[actor])
      for genre in movies[key].genres:
        nodedict[genre].neighbors.append(nodedict[key])
        nodedict[key].neighbors.append(nodedict[genre])
