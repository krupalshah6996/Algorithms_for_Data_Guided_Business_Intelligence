...
#print(temp)
for (i in temp){
  probabilities[i] = max(prob[,i])
}
prob_sorted <- sort(probabilities,decreasing = TRUE, index.return = TRUE)
probs_top_100 <- prob_sorted$x[1:100]
index_probs = prob_sorted$ix[1:100]
words_top_100 <- LDA_model@terms[index_probs]
#print(top100_words)
```
...
```{r}
train_set <- AssociatedPress[1:1500,]
test_set <- AssociatedPress[-(1:1500),]
lda <- c()
perplexity_ <- c()
for(i in seq(20,200,20)){
  lda_model <- LDA(train_set, control=list(iter=20, seed=SEED), k=i, method="Gibbs")
  lda <- c(lda,lda_model)
  perplexity_ <- c(perplexity_,perplexity(lda_model,test_set))
}
plot(seq(20,200,20),perplexity_,xlab="Number of Topics",ylab="Perplexity Model")
```
...
```{r}
# YOUR CODE HERE
train_set <- yelp_matrix[1:1000,]
test_set <- yelp_matrix[-(1:1000),]
lda <- c()
npmi <- c()
top_N <- 15
for(i in seq(10,100,10)){
  lda_model <- LDA(train_set, control=list(iter=100, seed=SEED), k=i, method="Gibbs")
  lda <- c(lda,lda_model)
  npmi <- c(npmi, compute_c(lda_model@beta,test_set,top_N,method = "NPMI"))
}
plot(seq(10,100,10),npmi,xlab="Number of Topics",ylab="NPMI")
...
# YOUR CODE HERE
lcp <- c()
for(lda_model in lda){
  lcp <- c(lcp, compute_c(lda_model@beta,test_set,top_N,method = "LCP"))
}
plot(seq(10,100,10),lcp,xlab="Number of Topics",ylab="LCP")
```
...
```{r}
# YOUR CODE HERE
yelp_10000 = yelp_matrix[1:10000,]
lda <- c()
for(i in seq(10,100,10)){
  lda_model <- LDA(yelp_10000, control=list(iter=10, seed=SEED), k=i, method="Gibbs")
  lda <- c(lda,lda_model)
}
```
...
for(lda_model in lda){
  ...
  max_prob = c()
  for (i in 1:nrow(pos_prob)){
    temp_max_prob <- max(pos_prob[i,])
    #print(temp_max_prob)
    for (j in 1:ncol(pos_prob)){
      #print(j)
      #print(pos_prob[i,2])
      if(pos_prob[i,j] == temp_max_prob){
        max_prob = c(max_prob,j)
        break
      }
    }
    #print(max_prob)
  }
  clusters[[count]] = max_prob
  count = count + 1
}
print(clusters[[2]][3])
```
...
```{r}
# YOUR CODE HERE
purity = function(cluster_assignments, label_vec){
  length_clusters = length(cluster_assignments)
  unique_clusters = length(unique(cluster_assignments))
  cluster_dict <- vector(mode = "list", length = unique_clusters)
  names(cluster_dict) <- c(1:unique_clusters)
  for(i in 1:unique_clusters){
    cluster_dict[[i]] <- list()
  }
  for(doc in seq(length_clusters)){
    id = cluster_assignments[doc]
    cluster_dict[[id]] <- c(cluster_dict[[id]], label_vec[doc])
  }
  total = 0
  #print(cluster_dict)

  for(i in 1:unique_clusters){
    unlist_cluster = unlist(cluster_dict[[i]])
    unique_ass = length(unique(unlist_cluster))
    temp_vector <- rep(0,length = unique_ass)
    #print(unlist_cluster)
    for (j in unlist_cluster){
      #print(temp_vector[i])
      temp_vector[j] = temp_vector[j] + 1
    }
    temp_max = 0
    for (v in temp_vector){
      if(!is.na(v) & v > temp_max){
        temp_max = v
      } 
    }
...
for(i in 1:10){
  ...
  purity_vector <- c(purity_vector, purity(clusters[[i]],labels))
}
#print(purity_vector)
plot(seq(10,100,10),purity_vector,xlab="No. of Topics",ylab="Purity")
```
